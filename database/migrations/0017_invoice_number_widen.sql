-- Migration 0017: Widen invoice_number from char(10) to char(20)
-- Affects 31 tables + logistics_invoice_reminders.provisional_invoice_number
-- 4 mixed-case FKs eliminated; 24 lowercase FKs re-added
-- 18 views rebuilt; 4 function local variables updated

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM schema_migrations WHERE script_name = '0017_invoice_number_widen') THEN
    RAISE NOTICE 'Migration 0017 already applied, skipping.';
    RETURN;
  END IF;

  -- ============================================================
  -- STEP 1: Drop dependent views
  -- invoice_contracts_view depends on invoice_details, so drop it first
  -- ============================================================
  ...
  -- Mixed-case: eliminated permanently (not re-added)
  -- ============================================================
  ALTER TABLE public.final_invoice_payments         DROP CONSTRAINT IF EXISTS "FK_FINAL_IN_FININVPAY_FINAL_IN";
  ALTER TABLE public.final_invoice_stocks            DROP CONSTRAINT IF EXISTS "FK_FINAL_IN_FININVSTO_FINAL_IN";
  ALTER TABLE public.forex_hedges_invoices_link      DROP CONSTRAINT IF EXISTS "FK_forex_hedges_invoices_link_invoices";
  ALTER TABLE public.invoice_stocks                  DROP CONSTRAINT IF EXISTS "FK_INVOICE__REF_99924_INVOICE";

  -- Lowercase: drop to re-add after widening
  ALTER TABLE public.containers                             DROP CONSTRAINT IF EXISTS fk_containers_invoice;
  ...
  ALTER TABLE public.creditdebit_note                       DROP CONSTRAINT IF EXISTS fk_crdrnote_invoice;
  ALTER TABLE public.final_invoice                          DROP CONSTRAINT IF EXISTS fk_finalinv_provinv;
  ALTER TABLE public.final_invoice_charges                  DROP CONSTRAINT IF EXISTS fk_fininvchrg_finalinv;
  ALTER TABLE public.final_invoice_details_1                DROP CONSTRAINT IF EXISTS fk_fininvdets1_invoice;
  ALTER TABLE public.final_invoice_details_2                DROP CONSTRAINT IF EXISTS fk_fininvdets2_invoice;
  ALTER TABLE public.final_invoice_payments                 DROP CONSTRAINT IF EXISTS fk_final_in_fininvpay_final_in;
  ALTER TABLE public.final_invoice_stocks                   DROP CONSTRAINT IF EXISTS fk_final_in_fininvsto_final_in;
  ALTER TABLE public.forex_hedges_invoices_link             DROP CONSTRAINT IF EXISTS fk_forex_hedges_invoices_link_invoices;
  ALTER TABLE public.invoice_charges                        DROP CONSTRAINT IF EXISTS fk_invchrg_invoice;
  ALTER TABLE public.invoice_containers                     DROP CONSTRAINT IF EXISTS fk_invoice_stocks_tariffs_invoice_stocks;
  ALTER TABLE public.invoice_details_1                      DROP CONSTRAINT IF EXISTS fk_invdets1_invoice;
  ALTER TABLE public.invoice_details_2                      DROP CONSTRAINT IF EXISTS fk_invdets2_invoice;
  ALTER TABLE public.invoice_payment_detail                 DROP CONSTRAINT IF EXISTS fk_invoice_payment_details_invoice;
  ALTER TABLE public.invoice_return                         DROP CONSTRAINT IF EXISTS fk_invoice_return_invoice;
  ALTER TABLE public.invoice_return_stocks                  DROP CONSTRAINT IF EXISTS fk_invoice_return_stocks_invoice_return;
  ALTER TABLE public.invoice_return_stocks                  DROP CONSTRAINT IF EXISTS fk_invoice_return_stocks_invoice_stocks;
  ALTER TABLE public.invoice_stocks                         DROP CONSTRAINT IF EXISTS fk_invoice__ref_99924_invoice;
  ALTER TABLE public.invoice_stocks_tariffs                 DROP CONSTRAINT IF EXISTS fk_invoice_stocks_tariffs_invoice_stocks;
  ALTER TABLE public.logistics_invoice_reminders            DROP CONSTRAINT IF EXISTS fk_logistics_invoice_reminders_creditdebit_note;
  ALTER TABLE public.provisional_invoice_details_2_archive  DROP CONSTRAINT IF EXISTS fk_invdets2_invoice;
  ALTER TABLE public.reserves_crdr                          DROP CONSTRAINT IF EXISTS fk_reserves_crdr_crdr;

  -- ============================================================
  -- STEP 3: Widen invoice_number to char(20) in 31 tables
  -- ============================================================
  ALTER TABLE public.bean_reception_group                  ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.containers                            ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.crdr_note_details                     ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.crdr_note_payments                    ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.creditdebit_note                      ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.doc_logging                           ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.document_tracking                     ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.final_invoice                         ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.final_invoice_charges                 ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.final_invoice_details_1               ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.final_invoice_details_2               ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.final_invoice_payments                ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.final_invoice_stocks                  ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.forex_hedges_invoices_link            ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice                               ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_charges                       ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_containers                    ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_details_1                     ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_details_2                     ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_history                       ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_payment_detail                ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_payments                      ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_return                        ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_return_stocks                 ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_stocks                        ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.invoice_stocks_tariffs                ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.phys_position_history                 ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.provisional_invoice_archive           ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.provisional_invoice_details_2_archive ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.reserves_crdr                         ALTER COLUMN invoice_number TYPE char(20);
  ALTER TABLE public.shipment_history                      ALTER COLUMN invoice_number TYPE char(20);
  -- logistics_invoice_reminders uses a different column name
  ALTER TABLE public.logistics_invoice_reminders           ALTER COLUMN provisional_invoice_number TYPE char(20);

  -- ============================================================
  -- STEP 4: Update function local variable ls_invoice_number char(10) -> char(20)
  -- ============================================================
  UPDATE pg_catalog.pg_proc
  SET prosrc = replace(prosrc, 'ls_invoice_number char(10)', 'ls_invoice_number char(20)')
  WHERE proname IN (
    'sp_list_allocated_contracts_unpaid_sales_invoices_fixed',
    'sp_list_allocated_contracts_unpaid_sales_invoices_unfixed',
    'sp_sopex_get_sum_invoice_value',
    'sp_sopex_sum_all_stocks_from_stock_alloc'
  )
  AND prosrc LIKE '%ls_invoice_number char(10)%';

  -- ============================================================
  -- STEP 5: Re-add 24 lowercase FK constraints
  -- ============================================================
  ALTER TABLE public.containers
    ADD CONSTRAINT fk_containers_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number);
  ...
  ALTER TABLE public.creditdebit_note
    ADD CONSTRAINT fk_crdrnote_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.final_invoice
    ADD CONSTRAINT fk_finalinv_provinv
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.final_invoice_charges
    ADD CONSTRAINT fk_fininvchrg_finalinv
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.final_invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.final_invoice_details_1
    ADD CONSTRAINT fk_fininvdets1_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.final_invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.final_invoice_details_2
    ADD CONSTRAINT fk_fininvdets2_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.final_invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.final_invoice_payments
    ADD CONSTRAINT fk_final_in_fininvpay_final_in
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.final_invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.final_invoice_stocks
    ADD CONSTRAINT fk_final_in_fininvsto_final_in
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.final_invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.forex_hedges_invoices_link
    ADD CONSTRAINT fk_forex_hedges_invoices_link_invoices
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number)
      ON DELETE CASCADE;

  ALTER TABLE public.invoice_charges
    ADD CONSTRAINT fk_invchrg_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.invoice_containers
    ADD CONSTRAINT fk_invoice_stocks_tariffs_invoice_stocks
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.invoice_details_1
    ADD CONSTRAINT fk_invdets1_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.invoice_details_2
    ADD CONSTRAINT fk_invdets2_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.invoice_payment_detail
    ADD CONSTRAINT fk_invoice_payment_details_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.invoice_return
    ADD CONSTRAINT fk_invoice_return_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number);

  ALTER TABLE public.invoice_return_stocks
    ADD CONSTRAINT fk_invoice_return_stocks_invoice_return
      FOREIGN KEY (invoice_type, client, invoice_number, return_id)
      REFERENCES public.invoice_return(invoice_type, client, invoice_number, return_id);

  ALTER TABLE public.invoice_return_stocks
    ADD CONSTRAINT fk_invoice_return_stocks_invoice_stocks
      FOREIGN KEY (invoice_type, client, invoice_number, contno, split, stock_id)
      REFERENCES public.invoice_stocks(invoice_type, client, invoice_number, contno, split, stock_id);

  ALTER TABLE public.invoice_stocks
    ADD CONSTRAINT fk_invoice__ref_99924_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.invoice(invoice_type, client, invoice_number);
  ...
  ALTER TABLE public.provisional_invoice_details_2_archive
    ADD CONSTRAINT fk_invdets2_invoice
      FOREIGN KEY (invoice_type, client, invoice_number)
      REFERENCES public.provisional_invoice_archive(invoice_type, client, invoice_number);
  ...
  INSERT INTO schema_migrations (script_name) VALUES ('0017_invoice_number_widen');
  RAISE NOTICE 'Migration 0017 applied successfully.';
END;
$$;

-- ============================================================
-- Recreate 18 dependent views
-- invoice_details must precede invoice_contracts_view
-- ============================================================
...
CREATE OR REPLACE VIEW public.invoice_charges_view AS
 SELECT params.systemdate,
    ...
    clause.longname AS invoice_end_clause_longname,
        CASE WHEN (params.coaddr1 IS NULL) THEN ''::character varying ELSE params.coaddr1 END AS our_addr1,
        CASE WHEN (params.coaddr2 IS NULL) THEN ''::character varying ELSE params.coaddr2 END AS our_addr2,
        CASE WHEN (params.coaddr3 IS NULL) THEN ''::character varying ELSE params.coaddr3 END AS our_addr3,
        CASE WHEN (params.coaddr4 IS NULL) THEN ''::character varying ELSE params.coaddr4 END AS our_addr4,
        CASE WHEN (params.coaddr5 IS NULL) THEN ''::character varying ELSE params.coaddr5 END AS our_addr5,
        CASE WHEN (params.coaddr6 IS NULL) THEN ''::character varying ELSE params.coaddr6 END AS our_addr6,
    client.name AS client_name,
    client.longname AS client_longname,
        CASE WHEN (client.addr1 IS NULL) THEN ''::character varying ELSE client.addr1 END AS client_addr1,
        CASE WHEN (client.addr2 IS NULL) THEN ''::character varying ELSE client.addr2 END AS client_addr2,
        CASE WHEN (client.addr3 IS NULL) THEN ''::character varying ELSE client.addr3 END AS client_addr3,
        CASE WHEN (client.addr4 IS NULL) THEN ''::character varying ELSE client.addr4 END AS client_addr4,
        CASE WHEN (client.addr5 IS NULL) THEN ''::character varying ELSE client.addr5 END AS client_addr5,
        CASE WHEN (client.addr6 IS NULL) THEN ''::character varying ELSE client.addr6 END AS client_addr6,
...
CREATE OR REPLACE VIEW public.invoices_posted_view AS
 SELECT invoice.invoice_number AS inv_no,
    ...
    CASE
        ...
        ELSE CASE WHEN (invoice.invoice_type = 'S'::bpchar) THEN 'Sales'::text ELSE 'Washouts'::text END
    END AS inv_type,
...
UNION
 SELECT final_invoice.invoice_number AS inv_no,
    ...
    CASE WHEN (final_invoice.invoice_type = 'P'::bpchar) THEN 'Purchases'::text ELSE 'Sales'::text END AS inv_type,
...
UNION
 SELECT creditdebit_note.crdr_number AS inv_no,
    ...
    CASE
        ...
        ELSE CASE WHEN (creditdebit_note.invoice_type = 'S'::bpchar) THEN 'Sales'::text ELSE 'Washouts'::text END
    END AS inv_type,
...
CREATE OR REPLACE VIEW public.invoices_view AS
 SELECT 'Provisional'::text AS type,
    invoice.company, invoice.pcentre, invoice.commodity, invoice.commodity_type, invoice.origin,
    invoice.invoice_number, invoice.invoice_type, invoice.invoice_date, invoice.client,
    client.longname, invoice.invoice_value, invoice.invoice_currency, invoice.description,
    CASE WHEN (invoice.posted_ledref IS NULL) THEN 'N'::text ELSE 'Y'::text END AS posted
   FROM public.invoice, public.client
  WHERE (invoice.client = client.code)
UNION ALL
 SELECT 'Final'::text AS type,
    final_invoice.company, final_invoice.pcentre, final_invoice.commodity, final_invoice.commodity_type, final_invoice.origin,
    final_invoice.invoice_number, final_invoice.invoice_type, final_invoice.invoice_date, final_invoice.client,
    client.longname, final_invoice.net_due AS invoice_value, final_invoice.invoice_currency, final_invoice.description,
    CASE WHEN (final_invoice.posted_ledref IS NULL) THEN 'N'::text ELSE 'Y'::text END AS posted
   FROM public.final_invoice, public.client
  WHERE (final_invoice.client = client.code)
UNION ALL
 SELECT 'Expense'::text AS type,
    expenses_summary.company, expenses_summary.pcentre, expenses_summary.commodity,
    expenses_summary.commodtype AS commodity_type, expenses_summary.origin,
    expenses_summary.expense_number AS invoice_number, expenses_summary.expense_type AS invoice_type,
    expenses_summary.expense_date AS invoice_date, expenses_summary.client,
    client.longname, expenses_summary.total_value AS invoice_value,
    expenses_summary.currency AS invoice_currency, expenses_summary.description,
    CASE WHEN (expenses_summary.posted_ledref IS NULL) THEN 'N'::text ELSE 'Y'::text END AS posted
   FROM public.expenses_summary, public.client
  WHERE (expenses_summary.client = client.code)
UNION ALL
 SELECT 'Cr/Dr Note '::text AS type,
    creditdebit_note.company, creditdebit_note.pcentre, creditdebit_note.commodity,
    creditdebit_note.commodtype AS commodity_type, creditdebit_note.origin,
    creditdebit_note.invoice_number, creditdebit_note.invoice_type, creditdebit_note.invoice_date, creditdebit_note.client,
    client.longname, creditdebit_note.total_value AS invoice_value,
    creditdebit_note.currency AS invoice_currency, creditdebit_note.description,
    CASE WHEN (creditdebit_note.posted_ledref IS NULL) THEN 'N'::text ELSE 'Y'::text END AS posted
   FROM public.creditdebit_note, public.client
...
CREATE OR REPLACE VIEW public.invoice_contracts_view AS
 SELECT params.systemdate,
    ...
    CASE WHEN (company.addr1 IS NULL) THEN ''::character varying ELSE company.addr1 END AS our_addr1,
    CASE WHEN (company.addr2 IS NULL) THEN ''::character varying ELSE company.addr2 END AS our_addr2,
    CASE WHEN (company.addr3 IS NULL) THEN ''::character varying ELSE company.addr3 END AS our_addr3,
    CASE WHEN (company.addr4 IS NULL) THEN ''::character varying ELSE company.addr4 END AS our_addr4,
    CASE WHEN (company.addr5 IS NULL) THEN ''::character varying ELSE company.addr5 END AS our_addr5,
    CASE WHEN (company.addr6 IS NULL) THEN ''::character varying ELSE company.addr6 END AS our_addr6,
    client.name AS client_name,
    client.longname AS client_longname,
    CASE WHEN (client.addr1 IS NULL) THEN ''::character varying ELSE client.addr1 END AS client_addr1,
    CASE WHEN (client.addr2 IS NULL) THEN ''::character varying ELSE client.addr2 END AS client_addr2,
