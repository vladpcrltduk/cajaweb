-- Migration 0021: widen several reference fields on accsummary
-- an_client:   char(8)  -> char(16)
-- an_allocref: char(10) -> char(20)
-- prov_inv_no: char(10) -> char(20)
-- crdr_inv_no: char(10) -> char(20)
-- exp_inv_no:  char(10) -> char(20)
-- fin_inv_no:  char(10) -> char(20)
-- No FK constraints involve these columns.
-- Dependent views: bal_anal_view, clt_accbals_view, clt_accbals_view_no_pcentre,
--                  forecast_report_outbooked_p_expenses_view, leds_view, nom_accbals_view

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.schema_migrations WHERE script_name = '0021_accsummary_fields_widen') THEN
    RAISE NOTICE 'Migration 0021 already applied, skipping.';
    RETURN;
  END IF;

  -- STEP 1: Drop all views that reference accsummary
  ...
  ALTER TABLE public.accsummary ALTER COLUMN an_client   TYPE char(16);
  ALTER TABLE public.accsummary ALTER COLUMN an_allocref TYPE char(20);
  ALTER TABLE public.accsummary ALTER COLUMN prov_inv_no TYPE char(20);
  ALTER TABLE public.accsummary ALTER COLUMN crdr_inv_no TYPE char(20);
  ALTER TABLE public.accsummary ALTER COLUMN exp_inv_no  TYPE char(20);
  ALTER TABLE public.accsummary ALTER COLUMN fin_inv_no  TYPE char(20);

  -- STEP 3: Record migration
  INSERT INTO public.schema_migrations (script_name) VALUES ('0021_accsummary_fields_widen');
  RAISE NOTICE 'Migration 0021 applied successfully.';
END;
$$;
