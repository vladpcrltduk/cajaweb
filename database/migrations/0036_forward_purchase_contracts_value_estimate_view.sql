-- ============================================================
-- forward_purchase_contracts_value_estimate_view
-- Source: dba.forward_purchase_contracts_value_estimate_view
-- 19 UNION ALL branches (sort_flag '00'–'18'), one per rolling
-- month window relative to params.systemdate.
-- Branch '18' has no upper bound (18+ months onwards).
-- SAP alias chains resolved by inlining DATE_TRUNC arithmetic —
-- no subquery nesting required.
-- SAP→PG: IF…ENDIF→CASE; isnull→COALESCE; month()/year()→EXTRACT;
--   dateadd(month,N,x)→x+INTERVAL; dateformat→TO_CHAR('FMMonth YYYY');
--   cast(x as char)→x::text; date(x)→x::date;
--   comma-joins→explicit JOIN/CROSS JOIN.
-- ORDER BY: original positional refs 1,15,3,4 →
--   sort_flag, contract_currency, contno, split.
-- ============================================================

CREATE OR REPLACE VIEW public.forward_purchase_contracts_value_estimate_view AS
